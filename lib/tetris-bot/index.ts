...
import type { BotConfig, Snapshot, TetrisBot, Weights } from './types';
import type { SnapshotLayout } from './layout';
import { SnapshotView, computeStructSize, setPieceCells } from './layout';

export type { BotConfig, Snapshot, SnapshotEvent, TetrisBot, Weights } from './types';
export { EventType, PieceLetter, SpinKind } from './types';
...
function clampPPS(pps: number): number {
  ...
  return Math.min(20, Math.max(0.1, pps));
}
